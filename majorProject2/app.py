...
def train_recognizer(data_path):
    ...
    if len(faces) < 2:  # Ensure there are at least 2 images to train the recognizer
        st.warning("Not enough data to train the recognizer. Please add more faces.")
        return None
...
app_mode = st.sidebar.selectbox("Choose the app mode", ["Add Face", "Identify Face", "Live Face Recognition"])
...
if app_mode == "Add Face":
    ...
    st.header("Identify faces from an uploaded image")
    ...
    if uploaded_file:
        ...
        if face_recognizer and len(faces) > 0:
            for (x, y, w, h) in faces:
                ...
                st.write(f"Identified face: {name} with confidence: {confidence:.2f}")
        else:
            st.write("No faces detected or face recognizer is not trained.")

elif app_mode == "Live Face Recognition":
    ...
    while run:
        ...
        if face_recognizer and len(faces) > 0:
            for (x, y, w, h) in faces:
                face_roi = cv2.cvtColor(frame[y:y+h, x:x+w], cv2.COLOR_RGB2GRAY)
